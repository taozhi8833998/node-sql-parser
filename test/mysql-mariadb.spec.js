...
describe('mysql', () => {
  ...
  describe('select', () => {
    ...
    it('should throw error when args is not right', () => {
      ...
      expect(parser.astify.bind(parser, sql)).to.throw('Expected "!=", "#", "&", "&&", ",", "--", "/*", "<<", "<=", "<>", ">", ">=", ">>", "AND", "BETWEEN", "IN", "IS", "LIKE", "NOT", "ON", "OR", "OVER", "REGEXP", "RLIKE", "USING", "XOR", "div", "||", [ \\t\\n\\r], [%*/], [+\\-], [<-=], or [\\^|] but ")" found.')
      expect(parser.astify.bind(parser, 'select convert("");')).to.throw('Expected "!=", "#", "&", "&&", ",", "--", "/*", "<<", "<=", "<>", ">", ">=", ">>", "AND", "BETWEEN", "COLLATE", "IN", "IS", "LIKE", "NOT", "OR", "REGEXP", "RLIKE", "USING", "XOR", "div", "||", [ \\t\\n\\r], [%*/], [+\\-], [<-=], or [\\^|] but ")" found.')
      sql = 'SELECT AVG(Quantity,age) FROM table1;'
      expect(parser.astify.bind(parser, sql)).to.throw('Expected "#", "&", "(", ")", "--", "->", "->>", ".", "/*", "<<", ">>", "div", "||", [ \\t\\n\\r], [%*/], [+\\-], [A-Za-z0-9_$\\x80-￿], [A-Za-z0-9_:], or [\\^|] but "," found.')
    })
