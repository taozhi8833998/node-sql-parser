...
set_op
  = KW_UNION __ a:(KW_ALL / KW_DISTINCT)? __ bn:('BY'i __ 'NAME'i)? {
    // => 'union' | 'union all' | 'union distinct' | 'union by name' | 'union all by name'
    let result = 'union'
    if (a) result += ` ${a.toLowerCase()}`
    if (bn) result += ' by name'
    return result
  }
  / KW_INTERSECT {
    // => 'intersect'
  ...
  / KW_EXCEPT __ a:KW_ALL? {
    // => 'except' | 'except all'
    return a ? 'except all' : 'except'
  }
...
describe_stmt
  = k:('DESCRIBE'i / 'SHOW'i) __ t:table_name {
    // => AstStatement<describe_stmt_node>
    return {
      tableList: Array.from(tableList),
      columnList: columnListTableAlias(columnList),
      ast: {
...
summarize_stmt
  = 'SUMMARIZE'i __ t:table_name {
    // => AstStatement<summarize_stmt_node>
    return {
      tableList: Array.from(tableList),
      columnList: columnListTableAlias(columnList),
      ast: {
...
attach_stmt
  = 'ATTACH'i __ e:literal_string __ KW_AS __ n:ident_name __ t:(LPAREN __ ident_name __ RPAREN)? {
    // => AstStatement<attach_stmt_node>
    return {
      tableList: Array.from(tableList),
      columnList: columnListTableAlias(columnList),
      ast: {
...
copy_stmt
  = 'COPY'i __ LPAREN __ s:union_stmt __ RPAREN __ KW_TO __ f:literal_string __ opts:copy_options? {
    return {
      tableList: Array.from(tableList),
      columnList: columnListTableAlias(columnList),
      ast: {
  ...
  / 'COPY'i __ t:table_name __ KW_TO __ f:literal_string __ opts:copy_options? {
    // => AstStatement<copy_stmt_node>
    return {
      tableList: Array.from(tableList),
      columnList: columnListTableAlias(columnList),
      ast: {
  ...
  / 'COPY'i __ t:table_name __ KW_FROM __ f:literal_string __ opts:copy_options? {
    return {
      tableList: Array.from(tableList),
      columnList: columnListTableAlias(columnList),
      ast: {
...
copy_options
  = LPAREN __ head:copy_option tail:(__ COMMA __ copy_option)* __ RPAREN {
    return createList(head, tail)
  }

copy_option
  = k:ident_name __ v:(literal_string / ident_name / literal_bool / literal_numeric) {
...
column_clause
  = STAR __ k:('EXCLUDE'i / 'REPLACE'i) __ LPAREN __ c:columns_list __ RPAREN tail:(__ COMMA __ column_list_item)* {
      // => column_list_item[]
...
column_list_item
  ...
  / 'COLUMNS'i __ LPAREN __ e:(literal_string / STAR) __ RPAREN {
      // => { type: 'expr'; expr: function; as: null; }
      const arg = typeof e === 'string' ? { type: 'star', value: '*' } : e
      return {
        type: 'expr',
        expr: {
          type: 'function',
          name: { name: [{ type: 'origin', value: 'COLUMNS' }] },
          args: { type: 'expr_list', value: [arg] },
        },
        as: null,
      }
    }
  / tbl:(ident_type __ DOT)? __ STAR __ 'EXCLUDE'i __ LPAREN __ c1:columns_list __ RPAREN __ 'REPLACE'i __ LPAREN __ c2:columns_list __ RPAREN {
      // => { expr: column_ref; exclude_list: columns_list; replace_list: columns_list; type: 'exclude_replace'; }
      ...
      return {
        ...
        type: 'exclude_replace',
        ...getLocationObject(),
      }
    }
  / tbl:(ident_type __ DOT)? __ STAR __ k:('EXCLUDE'i / 'REPLACE'i) __ LPAREN __ c:columns_list __ RPAREN {
      // => { expr: column_ref; expr_list: columns_list; type: 'exclude' | 'replace'; }
      const table = tbl && tbl[0] || null
      columnList.add(`select::${table ? table.value : null}::(.*)`)
      return {
        expr_list: c,
...
join_op
  ...
  / 'ASOF'i __ KW_JOIN { /* => 'ASOF JOIN' */ return 'ASOF JOIN'; }
  / 'ASOF'i __ KW_LEFT __ KW_JOIN { /* => 'ASOF LEFT JOIN' */ return 'ASOF LEFT JOIN'; }
  / 'CROSS'i __ KW_JOIN { /* => 'CROSS JOIN' */ return 'CROSS JOIN'; }
  / 'POSITIONAL'i __ KW_JOIN { /* => 'POSITIONAL JOIN' */ return 'POSITIONAL JOIN'; }
  / 'SEMI'i __ KW_JOIN { /* => 'SEMI JOIN' */ return 'SEMI JOIN'; }
  / 'ANTI'i __ KW_JOIN { /* => 'ANTI JOIN' */ return 'ANTI JOIN'; }
...
group_by_clause
  = KW_GROUP __ KW_BY __ KW_ALL {
    // => { columns: [{ type: 'origin'; value: 'ALL' }]; modifiers: literal_string[]; }
    return {
      columns: [{ type: 'origin', value: 'ALL' }]
    }
  }
...
partition_by_clause
  = KW_PARTITION __ KW_BY __ LPAREN __ head:or_and_where_expr tail:(__ COMMA __ or_and_where_expr)* __ RPAREN {
    /* => { type: 'expr'; expr: expr; parentheses: true }[] */
    const items = [head, ...tail.map(t => t[3])]
    return [{ type: 'expr', expr: { type: 'expr_list', value: items, parentheses: true } }];
  }
...
order_by_clause
  = KW_ORDER __ KW_BY __ KW_ALL __ d:(KW_DESC / KW_ASC)? {
    // => order_by_list
    return [{ expr: { type: 'origin', value: 'ALL' }, type: d }];
  }
...
replace_insert
  = KW_INSERT __ KW_OR __ KW_REPLACE { /* => 'insert or replace' */ return 'insert or replace'; }
  / KW_INSERT __ KW_OR __ KW_IGNORE { /* => 'insert or ignore' */ return 'insert or ignore'; }
...
lambda_expr
  = LPAREN __ head:ident_name tail:(__ COMMA __ ident_name)* __ RPAREN __ '->' __ body:_expr {
    // => { type: 'lambda'; args: { value: expr[]; parentheses: boolean }; expr: expr; }
    const argList = [head, ...tail.map(t => t[3])].map(a => ({ type: 'column_ref', table: null, column: a }))
    return {
      type: 'lambda',
      args: { value: argList, parentheses: true },
      expr: body,
    }
  }
  / arg:ident_name __ '->' __ body:_expr {
    // => { type: 'lambda'; args: { value: expr[]; parentheses: boolean }; expr: expr; }
    return {
      type: 'lambda',
      args: { value: [{ type: 'column_ref', table: null, column: arg }], parentheses: false },
      expr: body,
    }
  }
...
is_op_right
  = KW_IS __ KW_NOT __ KW_DISTINCT __ KW_FROM __ right:additive_expr {
      // => { op: 'IS NOT DISTINCT FROM'; right: additive_expr; }
      return { op: 'IS NOT DISTINCT FROM', right: right };
  }
...
struct_literal
  = '{' __ head:struct_field tail:(__ COMMA __ struct_field)* __ '}' {
    // => { type: 'struct_value'; fields: struct_field[]; }
    return {
      type: 'struct_value',
      fields: createList(head, tail),
    }
  }

struct_field
  = k:(literal_string / ident_name) __ ':' __ v:expr {
    // => { type: 'struct_field'; key: string; value: expr; }
    return {
      type: 'struct_field',
      key: typeof k === 'object' ? k.value : k,
      value: v,
    }
  }
...
aggr_func
  = e:(aggr_fun_count / aggr_fun_smma / aggr_array_agg / aggr_fun_list) __ f:aggr_filter? {
...
aggr_array_agg
  = pre:(ident __ DOT)? __ name:(KW_ARRAY_AGG / KW_STRING_AGG) __ LPAREN __ arg:distinct_args __ RPAREN __ bc:over_partition? {
...
aggr_fun_list
  = name:('LIST'i / 'LIST_AGG'i / 'LISTAGG'i) __ LPAREN __ arg:distinct_args __ RPAREN __ bc:over_partition? {
    // => { type: 'aggr_func'; args:count_arg; name: 'LIST';  }
      return {
        type: 'aggr_func',
        name: name.toUpperCase(),
        args: arg,
...
cast_data_type
  = p:'"'? t:data_type s:'"'? ab:(__ LBRAKE __ RBRAKE)? {
    // => data_type & { quoted?: string; array?: object }
    if ((p && !s) || (!p && s)) throw new Error('double quoted not match')
    if (p && s) t.quoted = '"'
    if (ab) t.array = { dimension: 1 }
    return t
  }
...
literal_array
  ...
  / LBRAKE __ c:expr_list? __ RBRAKE {
    // => { expr_list: expr_list; type: 'array'; brackets: true }
    return {
      expr_list: c || { type: 'origin', value: '' },
      type: 'array',
      keyword: '',
...
duckdb_numeric_type
  = t:('HUGEINT'i / 'UHUGEINT'i / 'UBIGINT'i / 'UINTEGER'i / 'USMALLINT'i / 'UTINYINT'i) !ident_start {
    // => data_type
    return { dataType: t.toUpperCase() }
  }

duckdb_binary_type
  = 'BLOB'i !ident_start { /* => data_type */ return { dataType: 'BLOB' }; }

duckdb_composite_type
  = 'STRUCT'i __ LPAREN __ head:struct_type_field tail:(__ COMMA __ struct_type_field)* __ RPAREN {
    // => data_type
    return { dataType: 'STRUCT', fields: createList(head, tail) }
  }
  / 'MAP'i __ LPAREN __ k:data_type __ COMMA __ v:data_type __ RPAREN {
    // => data_type
    return { dataType: 'MAP', key_type: k, value_type: v }
  }
  / 'LIST'i __ LPAREN __ t:data_type __ RPAREN {
    // => data_type
    return { dataType: 'LIST', element_type: t }
  }
  / 'UNION'i __ LPAREN __ head:struct_type_field tail:(__ COMMA __ struct_type_field)* __ RPAREN {
    // => data_type
    return { dataType: 'UNION', fields: createList(head, tail) }
  }

struct_type_field
  = n:ident_name __ t:data_type {
    // => { name: string; type: data_type; }
