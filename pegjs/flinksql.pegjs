{
  ...
  // FlinkSQL specific helper function
...
join_op
  ...
  / ko:(KW_CROSS / KW_OUTER) __ KW_APPLY { /* => '[ CROSS | OUTER ] APPLY' */ return `${ko.toUpperCase()} APPLY`; }
...
// FlinkSQL specific: LOGIC_OPERATOR without XOR
LOGIC_OPERATOR = OPERATOR_CONCATENATION / OPERATOR_AND

// FlinkSQL specific symbols
WELL_ARROW = '#>'
DOUBLE_WELL_ARROW = '#>>'
IMPLIES_ARROW = '=>'

// Identifier patterns (FlinkSQL with unicode support and dash in ident_part)
...
@import 'common/datatype/row.pegjs'
@import 'common/value/core.pegjs'

// FlinkSQL specific: interval_unit with plural forms
interval_unit
  = KW_UNIT_YEAR
  / KW_UNIT_MONTH
  / KW_UNIT_DAY
...
array_type
  = t:(KW_ARRAY / KW_MAP) __ LANGLE_BRACKET __ a:data_type_list __ RANGLE_BRACKET {
    return {
      dataType: t,
      angle_brackets: true,
      expr: {
