...
const surportedTypes = ['select', 'delete']
...
function deleteToSQL(stmt) {
  ...
  console.warn('delete', Array.isArray(stmt.tables))
...
function onlySelectSupported(expr) {
  const ast = expr && expr.ast ? expr.ast : expr
  if (!surportedTypes.includes(ast.type)) throw new Error('Only SELECT statements supported at the moment')
}

module.exports = function toSQL(ast) {
  if (Array.isArray(ast)) {
    ast.forEach(onlySelectSupported)
    return multipleToSQL(ast)
  }
  onlySelectSupported(ast)
